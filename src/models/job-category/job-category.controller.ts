...
export class JobCategoryController{
    static postJobCategoryController=catchAsync(async(req:Request,res:Response)=>{
        ...
        throw new AppError(400,"something went wrong ","Failed to create Job Category");
    })

   static updateJobCategoryController=catchAsync(async(req:Request,res:Response)=>{
        const payload=await req?.body;
        const {name}= payload;
        const OR : any[]= Object.entries(name).map(([key,value])=>
        ({name:{path:[key] , equals:value}}))
        ...
        throw new AppError(400,"something went wrong ","Failed to update Job Category");
    })

    static findJobCategoryControllerPublic = catchAsync(async (req: Request, res: Response) => {
